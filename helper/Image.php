<?php
class TMC_Helper_Image {

	/**
	 * @var int Default output image quality
	 */
	public		$quality	= 80;
        protected $allowType = array('image/gif','image/jpeg','image/png');
	...
	function __destruct () {
        ...
        public function isImage($filename) {
            ...
            if(in_array($info['mime'],$this->allowType)) {
                return true;
            }
            return false;
        }
	...
	function overlay ($overlay_file, $position = 'center', $opacity = 1, $x_offset = 0, $y_offset = 0) {
		// Load overlay image
		$overlay	= new TMC_Helper_Image($overlay_file);
		// Convert opacity
		$opacity	= $opacity * 100;
		// Determine position
